...
// ── TÍNH NĂNG CHECK-IN VÉ BẰNG MÃ QR & NHẬP THỦ CÔNG ────
let html5QrReader = null;

window.submitManualCheckin = function () {
    const input = document.getElementById("manualTicketCodeInput");
    const code = input ? input.value.trim() : "";
    if (!code) {
        showToast("Vui lòng nhập mã vé!", "error");
        return;
    }
    _executeCheckin(code);
};
...
window.toggleCameraScanner = function () {
    ...
    if (container.style.display === "none") {
        ...
        btn.textContent = "⏹️ Tắt Camera Quét QR";
...
function _startScanner() {
    if (!window.Html5Qrcode) {
        showToast("Lỗi: Thư viện quét QR chưa được tải!", "error");
        return;
    }
    
    // Clear checkin result
    document.getElementById("checkinResultContainer").style.display = "none";
    
    html5QrReader = new Html5Qrcode("qrReader");
    html5QrReader.start(
        ...
        (decodedText) => {
            ...
            if (decodedText.includes("Mã vé:")) {
                ...
                const line = lines.find(l => l.includes("Mã vé:"));
                if (line) {
                    ticketCode = line.replace("Mã vé:", "").trim();
                }
            }
            
            _executeCheckin(ticketCode);
            stopCameraScanner();
        },
        (errorMessage) => {
            // ignore
        ...
        showToast("Không thể khởi động camera quét QR!", "error");
        stopCameraScanner();
    });
}
...
window.stopCameraScanner = function () {
    ...
    if (btn) {
        btn.textContent = "📷 Bật Camera Quét QR";
...
function _executeCheckin(ticketCode) {
    const resContainer = document.getElementById("checkinResultContainer");
    resContainer.style.display = "block";
    resContainer.innerHTML = `
        <div class="overview-map" style="max-width:540px; padding:20px 24px; text-align:center;">
            <div class="spinner" style="margin: 20px auto;"></div>
            <p style="color:#555;">Đang đối chiếu & xác thực mã vé...</p>
        </div>
    `;
    
    apiFetch(`/ghe/checkin?ticketCode=${encodeURIComponent(ticketCode)}`, {
        method: "POST"
    })
    .then(res => {
        showToast("✅ Check-in thành công!", "success");
        resContainer.innerHTML = `
            <div class="overview-map" style="max-width:540px; padding:24px 32px; border:2px solid #16a34a; background:#f0fdf4;">
                <div style="font-size:3.5rem; text-align:center; margin-bottom:12px;">✅</div>
                <h3 style="text-align:center; color:#15803d; font-size:1.3rem; margin:0 0 16px;">
                    CHECK-IN THÀNH CÔNG!
                </h3>
                <div style="font-size:.9rem; color:#555; line-height:1.8; border-top:1px dashed #bbf7d0; padding-top:16px;">
                    <div>🎫 Mã vé: <strong style="color:#111; font-family:monospace;">${ticketCode}</strong></div>
                    <div>🎪 Sự kiện: <strong style="color:#111;">${res.tenSuKien || "—"}</strong></div>
                    <div>🏷️ Loại vé: <strong style="color:#111;">${res.tenVe || "—"}</strong></div>
                    <div>💺 Vị trí ghế: <span style="background:#dcfce7; color:#15803d; font-weight:700; padding:2px 8px; border-radius:6px; font-size:.85rem;">Khu ${res.khuVuc || "Tự do"}</span></div>
                    <div>👤 Mã ghế ID: <strong style="color:#111;">#${res.maGhe}</strong></div>
                    <div style="margin-top:8px; font-size:.78rem; color:#888; text-align:center; font-style:italic;">
                        Đã đổi trạng thái sang: ĐÃ CHECK-IN (DA_CHECKIN)
                    </div>
                </div>
            </div>
        `;
    ...
    .catch(err => {
        ...
        resContainer.innerHTML = `
            <div class="overview-map" style="max-width:540px; padding:24px 32px; border:2px solid #dc2626; background:#fef2f2;">
                <div style="font-size:3.5rem; text-align:center; margin-bottom:12px;">❌</div>
                <h3 style="text-align:center; color:#b91c1c; font-size:1.3rem; margin:0 0 16px;">
                    LỖI XÁC THỰC CHECK-IN!
                </h3>
                <div style="font-size:.9rem; color:#555; line-height:1.8; border-top:1px dashed #fecaca; padding-top:16px;">
                    <div style="color:#b91c1c; font-weight:700; font-size:1rem; margin-bottom:8px; text-align:center;">
                        ${err.message}
                    </div>
                    <div>🎫 Mã vé quét: <strong style="color:#111; font-family:monospace;">${ticketCode}</strong></div>
                    <div style="margin-top:8px; font-size:.78rem; color:#888; text-align:center; font-style:italic;">
                        Vui lòng kiểm tra lại vé hoặc liên hệ quản trị viên để giải quyết.
                    </div>
                </div>
            </div>
        `;
